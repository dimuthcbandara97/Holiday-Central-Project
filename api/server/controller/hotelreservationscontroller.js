  ...
  user
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while creating the hotel reservation.",
      });
    });
};
    ...
    HotelReservationsDb.findById(id)
      ...
      .catch((err) => {
        res.status(500).send({
          message:
            err.message ||
            "Some error occurred while retrieving the required user.",
        });
      });
  } else {
  ...
  HotelReservationsDb.find(query)
    .then((hotelReservations) => {
      if (hotelReservations.length === 0) {
        return res.status(404).send({
          message: "No hotel reservations matching the given criteria found.",
        });
      }
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while retrieving hotel reservations.",
      });
    });
};
  ...
  HotelReservationsDb.countDocuments(query)
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while retrieving the count of hotel reservations.",
      });
    });
};
...
exports.update = (req, res) => {
  ...
  HotelReservationsDb.findByIdAndUpdate(id, req.body, {
