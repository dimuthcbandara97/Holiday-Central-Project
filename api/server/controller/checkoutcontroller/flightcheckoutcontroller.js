let FlightCheckoutDb  = require('../../model/checkoutmodel/flightcheckoutmodel')
...
exports.update = (req, res) => {
  ...
  FlightCheckoutDb.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then(data => {
      if (!data) {
...
exports.delete = (req, res) => {
  ...
  const id = req.params.id
  FlightCheckoutDb.findByIdAndDelete(id)
    .then(data => {
      if (!data) {
        res.status(404).send({ message: "can not find user" })
      } else {
...
exports.countBy = (req, res) => {
  ...
  FlightCheckoutDb.countDocuments(query)
    .then(count => {
