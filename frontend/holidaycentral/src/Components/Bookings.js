...
const Booking = () => {
  ...
  useEffect(() => {
    // Cart Flight
    axios.get('http://localhost:4000/api/checkout/flight')
      .then(response => {
      ...
      // Cart Hotel
      axios.get('http://localhost:4000/api/checkout/hotel')
      .then(response => {
      ...
      // // Cart Package
      axios.get('http://localhost:4000/api/checkout/package')
      .then(response => {
      ...
      <div class="container m-3 ps-3">
        ...
        <div key={item.id}>
          <tr>
                ...
                <td> Departure Destination: {item.departure_destination}</td>
...
{dataFlight.map(item => (
        <div key={item.id}>
          ...
          <td> Cabin Class: {item.cabin_class}</td>
                ...
                <td> Indian Meal Preferences: {item.meal_preferences.indian}</td>
                <td> Sri Lankan Meal Preferences: {item.meal_preferences.sri_lankan}</td>
                <td> Italina Meal Preferences: {item.meal_preferences.italian}</td>
              </tr>
          {/* <h3>{item.agent_name}</h3>
          <p>{item.duration ? 'Completed' : 'Not completed'}</p> */}
        </div>
        ...
        <div key={item.id}>
          ...
          <td> Deluxe: {item.room_selection.deluxe ? 'Want' : 'Dont Want'} </td>
          <td> Super Deluxe: {item.room_selection.super_deluxe ? 'Want' : 'Dont Want'} </td>
          <td> Suite: {item.room_selection.suite ? 'Want' : 'Dont Want'} </td>
                <td> Full Board {item.board_basis.full_board ? 'Want' : 'Dont Want'}</td>
                <td> Bread Required {item.bread ? 'Want' : 'Dont Want'}</td>
                <td> Breakfast {item.breakfast ? 'Want' : 'Dont Want'}</td>
                <td> Pool: {item.facilities.pool ? 'Want' : 'Dont Want'}</td>
                <td> Kids Play Area: {item.facilities.kids_play_area ? 'Want' : 'Dont Want'}</td>
                <td> Beach Access: {item.facilities.beach_access ? 'Want' : 'Dont Want'}</td>
